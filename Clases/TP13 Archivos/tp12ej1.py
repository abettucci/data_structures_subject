...
1. Crear un diccionario que permita manejar la información de las ventas en cada zona de cada
vendedor.
2. Pasar la información a un archivo txt
3. Pasar la información a un archivo csv
4. Mostrar los archivos creados, verificando que existen haciendo uso del try - except
'''
...
def menu():
    opcionIngresada = int(input('''
        ...
        5. Pasar la información a un archivo txt
        6. Pasar la información a un archivo csv
        7. Ver archivo csv
        8. Ver archivo txt
        9. Finalizar
    ...
    while corriendo == True:
        if opcionIngresada == 1:
            ...
            with open('tp12ej1.txt','w') as txtfile:
                txtfile.write(str(dictVendedores))
            opcionIngresada = int(input('Ingrese una nueva accion: '))

        elif opcionIngresada == 6:
            with open('tp12ej1.csv','w', newline='') as csvfile:
                encabezados = nombres
                writer = csv.DictWriter(csvfile, fieldnames=encabezados)
                writer.writeheader()
                writer.writerow(dictVendedores)
            opcionIngresada = int(input('Ingrese una nueva accion: '))
        
        elif opcionIngresada == 7:
            try:
                with open('tp12ej1.csv','r',newline='') as csvfile:
                    reader = csv.DictReader(csvfile)
                    for heading in reader.fieldnames:
                        print(heading, end='\t')
                    print()
                    for linea in reader:
                        print(linea['Nicolas'],'\t',linea['Daniela'],'\t',linea['Maria'],'\t',linea['Francisco'])
            except FileNotFoundError:
                print('No existe el archivo csv buscado')
            opcionIngresada = int(input('Ingrese una nueva accion: '))

        elif opcionIngresada == 8:
            try:
                with open('tp12ej1.txt','r') as txtfile:
                    for linea in txtfile:
                        if (linea[-1] == '\n'):
                            linea = linea[:-1]
                        print(linea)
            except FileNotFoundError:
                print('No existe el archivo txt buscado')
            opcionIngresada = int(input('Ingrese una nueva accion: '))
